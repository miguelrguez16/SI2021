...
public class CursoController {
	...
	public void initController() {
		...
		vista.getRellenarDatos().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				vista.getInicioInscripcion().setText("2020-02-002");
				vista.getFinInscripcion().setText("2021-02-002");
	...
	private void guardarCambios() {
		...
		try {
			...
			throw new ApplicationException("Error en el formato del numero");
		}finally {
		if(plazas != -10 || id!=-10) {
			//llamar al modelo guardar datos
			System.out.println("Datos modificados: " + id);
